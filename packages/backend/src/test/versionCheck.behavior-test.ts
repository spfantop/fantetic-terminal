...
assert.match(
  versionRoutes,
  /raw\.githubusercontent\.com\/\$\{GITHUB_REPO\}\/main\/package\.json/,
  'version fallback should use the root package metadata, which is the release version source',
);

assert.doesNotMatch(
  versionRoutes,
  /main\/VERSION/,
  'version fallback must not reference a VERSION file that is absent from the repository root',
);

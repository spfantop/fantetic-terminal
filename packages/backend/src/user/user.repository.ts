...
export class UserRepository {
  async findUserById(id: number): Promise<User | null> {
    ...
    const sql = 'SELECT id, username, hashed_password, two_factor_secret, system_role, status, auth_epoch, created_at, updated_at FROM users WHERE id = ?';
  ...
  async findUserByUsername(username: string): Promise<User | null> {
    const db = await getDbInstance();
    const sql = 'SELECT id, username, hashed_password, two_factor_secret, system_role, status, auth_epoch, created_at, updated_at FROM users WHERE username = ?';

    ...
    const heartbeatInterval = setInterval(() => {
        wss.clients.forEach((ws: WebSocket) => {
            ...
            if (extWs.isAlive === false) {
                extWs.missedHeartbeatCount = (extWs.missedHeartbeatCount ?? 0) + 1;
                if (extWs.missedHeartbeatCount >= HEARTBEAT_MISSED_LIMIT) {
                    console.log(`WebSocket 心跳检测：用户 ${extWs.username} (会话: ${extWs.sessionId}) 连续 ${extWs.missedHeartbeatCount} 次无响应，正在终止...`);
                    cleanupClientConnection(extWs.sessionId); // 使用会话 ID 清理
                    return extWs.terminate();
                }
            } else {
                extWs.missedHeartbeatCount = 0;
            }
            extWs.isAlive = false;
            extWs.ping(() => {});
        });

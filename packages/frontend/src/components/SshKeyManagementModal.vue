...
<template>
    <div ref="modalRootRef" class="fixed inset-0 bg-overlay flex justify-center items-center z-50 p-4">
        <div ref="modalContentRef" class="bg-background text-foreground p-6 rounded-lg shadow-xl border border-border w-full max-w-3xl max-h-[80vh] flex flex-col">

            <!-- Main Modal Content -->
            <div v-if="!isAddEditFormVisible" class="flex flex-col h-full">
                <h3 class="text-xl font-semibold text-center mb-4 flex-shrink-0 cursor-move select-none" @pointerdown="startDialogDrag">{{ t('sshKeys.modal.title') }}</h3>

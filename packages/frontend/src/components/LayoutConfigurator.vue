  ...
  <div ref="dialogRootRef" v-if="isVisible" class="fixed inset-0 bg-overlay flex justify-center items-center z-[1000]" @click.self="closeDialog">
    <div ref="dialogRef" class="layout-configurator-dialog bg-background text-foreground rounded-lg shadow-xl w-auto h-auto min-w-[800px] min-h-[600px] max-w-[95vw] max-h-[90vh] flex flex-col overflow-auto relative pointer-events-auto cursor-default">
      ...
      <main class="flex-grow p-6 overflow-y-auto grid grid-cols-[220px_1fr] gap-6 min-h-[450px]">
        ...
        <section class="flex flex-col overflow-y-auto border-r border-border pr-6 min-w-[200px]">
          ...
          >
            <template #item="{ element }: { element: PaneName }">
              <li class="layout-configurator-list-item flex items-center p-2 mb-2 border rounded cursor-grab transition-colors duration-150 active:cursor-grabbing text-sm">
        ...
        <div class="flex flex-col">
            ...
            <section class="flex flex-col min-w-[350px] flex-grow">
                ...
                <div class="flex items-center gap-2">
                   ...
                   <button
                     ...
                     :class="[
                       ...
                       layoutLockedBoolean ? 'layout-configurator-switch-on' : 'layout-configurator-switch-off'
                     ]"
                     role="switch"
              ...
              <div class="layout-configurator-preview flex-grow border-2 border-dashed rounded p-4 flex flex-col overflow-auto min-h-[250px]">
                <LayoutNodeEditor
              ...
              <div class="mt-4 flex gap-2">
                 <button @click="resetToDefault" class="layout-configurator-secondary-button py-2 px-4 rounded text-sm transition-colors duration-150 border">
            ...
            <div class="grid grid-cols-2 gap-6 border-t border-border pt-4 mt-4 min-h-[150px]">
                ...
                <section class="flex flex-col">
                    ...
                    <draggable
                        ...
                        tag="ul"
                        class="layout-configurator-drop-list list-none p-0 m-0 min-h-[120px] border border-dashed rounded p-2 flex-grow overflow-y-auto"
                    ...
                    >
                        <template #item="{ element, index }: { element: PaneName, index: number }">
                            <li class="layout-configurator-list-item flex items-center justify-between p-2 mb-2 border rounded cursor-grab transition-colors duration-150 active:cursor-grabbing text-sm">
      ...
      <footer class="p-4 border-t border-border flex justify-end gap-3 bg-header">
        <button @click="closeDialog" class="layout-configurator-secondary-button py-2 px-4 rounded text-sm transition-colors duration-150 border">{{ t('common.cancel', '取消') }}</button>
        <button @click="saveLayout" class="layout-configurator-primary-button py-2 px-4 rounded text-sm transition-colors duration-150 disabled:opacity-70 disabled:cursor-not-allowed" :disabled="!isModified">
...
.layout-configurator-dialog {
  --layout-configurator-surface: color-mix(in srgb, var(--app-bg-color) 86%, var(--header-bg-color));
  --layout-configurator-elevated: color-mix(in srgb, var(--header-bg-color) 82%, var(--app-bg-color));
  --layout-configurator-muted: color-mix(in srgb, var(--header-bg-color) 74%, var(--border-color));
  --layout-configurator-hover: color-mix(in srgb, var(--nav-item-active-bg-color) 68%, var(--header-bg-color));
  --layout-configurator-accent: var(--link-active-color);
  --layout-configurator-action-bg: var(--button-bg-color);
  --layout-configurator-action-hover: var(--button-hover-bg-color);
  --layout-configurator-action-text: var(--button-text-color);
  background: var(--layout-configurator-surface);
  border: 1px solid color-mix(in srgb, var(--border-color) 76%, transparent);
}

.layout-configurator-drag-handle {
  cursor: move;
  user-select: none;
}

.layout-configurator-drag-handle button {
  cursor: pointer;
}

.layout-configurator-dialog :where(button, i) {
  color: currentColor;
}

.layout-configurator-preview,
.layout-configurator-drop-list {
  border-color: color-mix(in srgb, var(--border-color) 74%, var(--text-color-secondary));
  background: var(--layout-configurator-elevated);
}

.layout-configurator-list-item {
  border-color: color-mix(in srgb, var(--border-color) 82%, transparent);
  background: var(--layout-configurator-muted);
  color: var(--text-color);
}

.layout-configurator-list-item:hover {
  border-color: color-mix(in srgb, var(--layout-configurator-accent) 38%, var(--border-color));
  background: var(--layout-configurator-hover);
}

.layout-configurator-switch-on {
  background: var(--layout-configurator-action-bg);
}

.layout-configurator-switch-off {
  background: color-mix(in srgb, var(--border-color) 72%, var(--header-bg-color));
}

.layout-configurator-primary-button,
.layout-configurator-secondary-button {
  border-color: color-mix(in srgb, var(--border-color) 72%, var(--text-color-secondary));
  font-weight: 600;
}

.layout-configurator-primary-button {
  background: var(--layout-configurator-action-bg);
  color: var(--layout-configurator-action-text);
}

.layout-configurator-primary-button:hover:not(:disabled) {
  background: var(--layout-configurator-action-hover);
}
...
.layout-configurator-secondary-button:hover:not(:disabled) {
  background: var(--layout-configurator-hover);
  border-color: color-mix(in srgb, var(--layout-configurator-accent) 36%, var(--border-color));
}
</style>

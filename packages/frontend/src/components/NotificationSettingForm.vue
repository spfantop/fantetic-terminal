...
debugLog('[NotificationSettingForm] Setup started.'); // Log setup start
const { t } = useI18n();
debugLog('[NotificationSettingForm] useI18n initialized.'); // Log i18n init
...
watch(() => props.initialData, (newData) => {
  debugLog('[NotificationSettingForm] Watch initialData triggered. New data:', newData); // Log initialData change
  if (newData) {
   ...
   debugLog('[NotificationSettingForm] Form data initialized/updated from initialData. Current channel_type:', formData.channel_type); // Log after init/update
}, { immediate: true });

// Watch channel type change to reset specific config
watch(() => formData.channel_type, (newType, oldType) => {
    debugLog(`[NotificationSettingForm] Watch channel_type changed from ${oldType} to ${newType}`); // Log channel type change
...
const getEventDisplayName = (event: NotificationEvent): string => {
    ...
    debugLog(`[NotificationSettingForm] Translating event display name for key: ${i18nKey}`); // Log event key translation attempt
...
const handleSubmit = async () => {
  debugLog('[NotificationSettingForm] handleSubmit called.'); // Log submit start

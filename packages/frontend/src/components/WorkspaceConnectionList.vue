...
let activeConnectionMenuDocument: Document | null = null;
...
let activeTagMenuDocument: Document | null = null;
...
const eventDocument = (event.target as Node | null)?.ownerDocument ?? listAreaRef.value?.ownerDocument ?? document;
...
// 添加全局点击监听器以关闭菜单
activeConnectionMenuDocument?.removeEventListener('click', closeContextMenu);
activeConnectionMenuDocument = eventDocument;
activeConnectionMenuDocument.addEventListener('click', closeContextMenu, { once: true });

// 使用 nextTick 获取菜单尺寸并调整位置以防止超出屏幕
nextTick(() => {
  const menuElement = eventDocument.querySelector('.context-menu') as HTMLElement;
  if (menuElement) {
...
const closeContextMenu = () => {
  ...
  activeConnectionMenuDocument?.removeEventListener('click', closeContextMenu);
  activeConnectionMenuDocument = null;
};
...
// 添加全局点击监听器以关闭菜单
activeTagMenuDocument?.removeEventListener('click', closeTagContextMenu);
activeTagMenuDocument = eventDocument;
activeTagMenuDocument.addEventListener('click', closeTagContextMenu, { once: true });

// 使用 nextTick 获取菜单尺寸并调整位置以防止超出屏幕
nextTick(() => {
  const menuElement = eventDocument.querySelector('.tag-context-menu') as HTMLElement;
  if (menuElement) {
    const menuRect = menuElement.getBoundingClientRect();
    const menuWindow = menuElement.ownerDocument.defaultView ?? eventDocument.defaultView ?? window;

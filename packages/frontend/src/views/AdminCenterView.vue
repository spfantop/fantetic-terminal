      ...
      <section class="admin-center-content" :aria-labelledby="`admin-section-${activeSection}`">
        ...
        <div v-if="activeSection === 'overview'" class="admin-overview">
          <div class="admin-overview-stats" :aria-busy="overviewLoading">
            <button v-for="stat in visibleStats" :key="stat.key" type="button" @click="selectSection(stat.section)"><span class="admin-overview-stat-icon" aria-hidden="true"><i :class="stat.icon"></i></span><span class="admin-overview-stat-label">{{ stat.label }}</span><strong v-if="overviewStats[stat.key] !== null">{{ overviewStats[stat.key] }}</strong><i v-else-if="overviewLoading" class="fas fa-spinner fa-spin overview-loading" aria-hidden="true"></i><em v-else>—</em></button>
          </div>
        </div>
...
const allItems = computed<AdminNavigationItem[]>(() => [
  { key: 'overview', label: t('adminCenter.sections.overview', '概览'), icon: 'fas fa-table-cells-large', roles: auditRoles },
...
.admin-overview{display:grid;gap:.75rem}.admin-overview-stats{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.65rem}.admin-overview-stats button{display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:.65rem;padding:.8rem;border:1px solid var(--border-color);border-radius:.55rem;background:var(--app-bg-color);color:var(--text-color);text-align:left;cursor:pointer}.admin-overview-stats button:hover,.admin-overview-stats button:focus-visible{border-color:color-mix(in srgb,var(--link-active-color) 42%,var(--border-color));background:var(--nav-item-active-bg-color);color:var(--link-active-color);outline:none}.admin-overview-stat-icon{display:grid;place-items:center;width:1.75rem;height:1.75rem;flex-shrink:0;border:1px solid color-mix(in srgb,var(--link-active-color) 24%,var(--border-color));border-radius:.45rem;background:var(--nav-item-active-bg-color);color:var(--link-active-color);font-size:.8rem}.admin-overview-stat-icon i{color:currentColor;font-size:inherit}.admin-overview-stat-label{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:.78rem;color:currentColor}.admin-overview-stats button strong{font-size:1.15rem}.admin-overview-stats button em{font-style:normal;color:var(--text-color-secondary)}

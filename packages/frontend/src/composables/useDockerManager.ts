...
export function createDockerManager(sessionId: string, wsDeps: DockerManagerDependencies, i18n: { t: (key: string, params?: any) => string }) {
    ...
    const { dockerManagerEnabledBoolean, dockerDefaultExpandBoolean } = storeToRefs(settingsStore);
    ...
    const startRefreshInterval = () => {
        if (!refreshInterval && dockerManagerEnabledBoolean.value && isDockerAvailable.value) {
            refreshInterval = setInterval(requestDockerStatus, 15000);
        }
    };

    // Request Docker status via WebSocket
    const requestDockerStatus = (force = false) => {
        if (!dockerManagerEnabledBoolean.value) {
            resetDockerDisabledState();
            return;
        }
        ...
        if (!force && !isDockerAvailable.value) {
            stopRefreshInterval();
            return;
        }
    ...
    const setupWsListeners = () => {
        ...
        const unsubRequestUpdate = onMessage('request_docker_status_update', (payload, message) => {
            if (message?.sessionId && message.sessionId !== sessionId) return;
            requestDockerStatus(true); // Trigger a status refresh immediately
        });
    ...
    const sendDockerCommand = (containerId: string, command: 'start' | 'stop' | 'restart' | 'remove') => {
        if (!dockerManagerEnabledBoolean.value) {
            console.warn(`[DockerManager ${sessionId}] Docker manager is disabled. Command '${command}' skipped.`);
            return;
        }
        if (!isConnected.value) {
    ...
    const resetDockerDisabledState = () => {
        containers.value = [];
        isLoading.value = false;
        error.value = null;
        isDockerAvailable.value = false;
    ...
    watch(isConnected, (newIsConnected, oldIsConnected) => {
        if (newIsConnected) {
            ...
            if (dockerManagerEnabledBoolean.value && layoutStore.usedPanes.has('dockerManager')) {
                // Connection established
                setupWsListeners();
                isDockerAvailable.value = true;
                requestDockerStatus(true); // Fetch initial status
    ...
    watch(dockerManagerEnabledBoolean, (enabled) => {
        const layoutStore = useLayoutStore();
        if (!enabled) {
            resetDockerDisabledState();
            return;

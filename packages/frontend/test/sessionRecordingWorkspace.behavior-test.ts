...
assert.match(component, /selectedRecordingIds/);
assert.match(component, /deleteSelectedRecordings/);
assert.match(component, /sessionRecording\.confirmDelete/);
assert.match(component, /timelineOffset\.value >= playbackDurationMs\.value/);
...
assert.match(component, /durationMs\.value/);
assert.doesNotMatch(component, /Math\.min\(1000, Math\.max\(0, event\.offsetMs - previous\)/);
assert.match(component, /replayProgressAnchorSpeed/);
...
assert.match(api, /async deleteMany\(ids: string\[\]\)/);
assert.match(api, /session-recordings.*data: request/);
